...
- Health monitoring dashboard with status indicators and real-time checks
...
**pricing.html** - Hero → 3 pricing tiers with detailed features (Starter free, Growth $10/user/month, Enterprise custom) → "All plans include" section → 8 FAQ items → CTA

**health.html** - **Admin-only** dashboard → Overall status summary → Frontend/Backend/Infrastructure service monitoring → Auto-refresh controls → Real-time status indicators
...
**Hidden Admin Page:** `/health.html` - Not linked in navigation, access via direct URL only
...
- **Service categories:**
  - Frontend Services (index, overview, features, pricing pages)
